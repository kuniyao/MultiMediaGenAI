...
def main():
    ...
    if len(translated_texts) != len(transcript_data_for_processing):
         task_logger.warning(f"Number of translated segments ({len(translated_texts)}) does not match original ({len(transcript_data_for_processing)}). Results might be incomplete or misaligned.")

    translated_md_filename = os.path.join(video_output_path, f"{file_basename_prefix}_translated_{args.target_lang}.md")
    translated_md_content = reconstruct_translated_markdown(transcript_data_for_processing, translated_texts, lang_code, source_type, args.target_lang, video_id_for_transcript, logger=task_logger)
    ...
    translated_srt_content = reconstruct_translated_srt(transcript_data_for_processing, translated_texts, logger=task_logger)
